...
from chess.common import ChessSquare, ChessColor, ChessPiece, ChessCastleSide
    ...
    def can_castle(self, descr: ChessCastleDescr) -> bool:
        ...
        for square, piece in kings:
            ...
            if (side == ChessCastleSide.kingside 
                and len(squares_to_check) != 3):
                return False
            elif (side == ChessCastleSide.queenside 
                  and len(squares_to_check) != 4):
                return False
            
            if (self.get_square(squares_to_check[-1])[1] 
                != ChessBitBoardPiece(color, ChessPiece.rook)):
                return False

            for sq in squares_to_check[:-1]:
